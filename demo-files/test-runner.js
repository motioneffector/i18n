  ...
  async runTests() {
    ...
    for (let i = 0; i < this.tests.length; i++) {
      ...
      try {
        ...
        const category = isIntegrity ? 'integrity' : 'library'

        output.innerHTML += `<div class="test-item" data-category="${category}"><span class="test-icon pass">✓</span><span class="test-name ${categoryClass}">${this.escapeHtml(test.name)}</span></div>`
      } catch (e) {
        failed++
        ...
        output.innerHTML += `<div class="test-item" data-category="${category}"><span class="test-icon fail">✗</span><div><div class="test-name ${categoryClass}">${this.escapeHtml(test.name)}</div><div class="test-error">${this.escapeHtml(e.message)}</div></div></div>`
      }
